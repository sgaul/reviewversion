...
levels(projections$AGE) <- list(
  "55 years and over" = c("8084","6064","7579","5559","90OV","7074","8589","6569"),
  "25 to 54 years" = c("4549","2529","4044","3539","5054","3034"),
  "0 to 24 years" = c("0004","0509","1014","4","509","2024","1519"))

age_chg <- within(merge(ddply(subset(projections, YEAR == 2025), .(AGE, Towngroup),summarise, value = sum(value)),
             ddply(subset(projections, YEAR == 2010), .(AGE, Towngroup),summarise, value = sum(value)),
             by=c("AGE","Towngroup")), {
  value <- value.x - value.y
})[,c("AGE","Towngroup","value")]

age_chg$AGE <- factor(age_chg$AGE, levels = c("0 to 24 years",
                                              "25 to 54 years",
                                              "55 years and over"))

age_chg$Towngroup <- factor(age_chg$Towngroup, levels = c("Hartford","Urban periphery","Suburban / rural"))

ggplot(data = age_chg, aes(x = Towngroup, y = value)) + 
  geom_bar(stat = 'identity') + 
  facet_wrap(~AGE, ncol = 3) + 
  labs(x = NULL, y = "Projected change in population (2010 - 2025)") +
  scale_y_continuous(labels = comma) + 
  theme_minimal()
```
...
```{r echo = FALSE, warning = FALSE, message = FALSE, fig.height = 6, fig.width = 13}
enroll <- read.csv("../regionalreport/data/all-schools-new.csv", 
...
ggplot(data = ddply(merge(subset(enroll, variable == "Total" & 
  ...
  geom_line(aes(group = School.Type, colour = School.Type), size = 1) +
  scale_y_continuous(labels = comma) +
...
C15002I = acs.fetch(geography = crcog.towns, table.number = "C15002I", col.names = "pretty", year = 2012)
C15002B = acs.fetch(geography = crcog.towns, table.number = "C15002B", col.names = "pretty", year = 2012)
C15002H = acs.fetch(geography = crcog.towns, table.number = "C15002H", col.names = "pretty", year = 2012)
...
library(gridExtra)
grid.arrange(rising, falling, nrow = 2, ncol = 1)
...
glr1 <- 
  ...
  scale_y_continuous(breaks = c(202,217,235,279), lim = c(180,300), 
                     labels = c("Basic","Proficient","Goal","Advanced")) +
...
glr2 <- 
  ...
  geom_hline(yintercept = c(202, 217, 235, 279), size = 2, alpha = 0.1) + 
  scale_x_continuous(breaks = c(2006,2009,2012)) +
  scale_y_continuous(breaks = NULL, lim = c(180,300)) +
  scale_colour_brewer(palette = "Dark2") + 
  theme_minimal() + 
  theme(legend.position = "top") +
  labs(x = NULL, y = NULL) + 
...
ggplot(data = ddply(merge(subset(voting, Type == "Municipal"),towns,by = "Town"),
                    ...
                    voters = sum(Number.Checked.as.Having.Voted, na.rm = T),
                    registered = sum(Names.on.Official.Check.List..Active., na.rm= T),
                    Percentage.Checked.as.Having.Voted =
                      weighted.mean(Percentage.Checked.as.Having.Voted,
                                    Names.on.Official.Check.List..Active.,na.rm = TRUE)),
       aes(x = as.Date(Date, format = "%m/%d/%Y"), 
           y = Percentage.Checked.as.Having.Voted, 
           group = Towngroup)) + 
  geom_line(aes(colour = Towngroup), size = 1) + 
...
#Load data
prek <- read.csv("../childcare/preKSDE.csv")
...
#Add column for town instead of district
prek$Town = gsub(prek$District.Name, pattern = " School District", replacement = "")
prek$No.pre.K = (1 - prek$Percentage.of.Kindergarteners.with.PreK.Experience / 100) * prek$Number.of.K.students

#Then add variable for town groups
prek <- merge(prek, towns, by = "Town")

prek_region <- ddply(prek,.(School.Year, Towngroup), summarise, 
                     value = weighted.mean(
                       Percentage.of.Kindergarteners.with.PreK.Experience, 
                       Number.of.K.students),
                     sum = sum(No.pre.K))

ggplot(data = prek_region, aes(x = School.Year, y = sum)) + 
  geom_line(aes(group = Towngroup, colour = Towngroup)) +
#  ylim(0,100) + 
  labs(x = NULL, y = "Students with no pre-K experience") +
  theme_minimal()
```
