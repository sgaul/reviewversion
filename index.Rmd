...
write.csv(subset(gradregion, !(Category %in% c("American Indian"))),
          "../rawdata/hsgraduation.csv", 
          row.names = FALSE)
```
...
grid.arrange(glr1, glr2, glr3, glr5, ncol = 3, nrow = 1)
...
vote <- 
ggplot(data = ddply(merge(voting,towns,by = "Town"),.(Year,Towngroup), summarise,
  ...
  geom_bar(stat = 'identity', width = 0.5) + 
  labs(x = NULL, y = "Percent of registered voters that voted", title = "Voter turnout") + 
  scale_y_continuous(labels = percent) + 
  coord_flip() + 
  theme_minimal()

write.csv(ddply(merge(voting,towns,by = "Town"),.(Year,Towngroup), summarise,
      Percentage.Checked.as.Having.Voted = 
        weighted.mean(Percentage.Checked.as.Having.Voted,
                      Names.on.Official.Check.List..Active.,na.rm = TRUE)),
