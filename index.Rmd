...
write.csv(subset(gh_test, period >= 1900),'slide4.csv', row.names = F)
```
...
write.csv(aging,'slide5.csv', row.names = F)
```
...
write.csv(languages_agg,'slide6.csv', row.names = F)
```
...
write.csv(subset(enroll, School.Year == "2010-11" & variable != "Total"), 
          "slide7.csv", row.names = FALSE)
...
gh$type <- ifelse(gh$city == "Hartford","Hartford","Suburbs")
gh_sum <- ddply(gh,.(type, period), summarise, sumpop = sum(population), sumpoor = sum(poppoverty))
...
write.csv(subset(gh_sum,period >= 1970),"slide9.csv", row.names = FALSE)
```
...
write.csv(income.tract,"slide10.csv", row.names = FALSE)
```
...
write.csv(cmt_subgroups_region,"slide11.csv", row.names = FALSE)
```
--- 
...
write.csv(absenteeism,"slide12.csv", row.names = FALSE)
```
--- 
...
write.csv(subset(hsgrad, !(Category %in% c("All Students"))),"slide13.csv", 
          row.names = FALSE)
```
--- 
...
write.csv(attainment.by.race,"slide14.csv", row.names = FALSE)
```
--- 
...
write.csv(subset(qcew, series == "Employees" & 
                       area != "Multiple locations or unknown" & 
                       area != "Statewide" & 
                       naicsector %in% c(44,52,54,61,62,31)),"slide15.csv", 
          row.names = FALSE)
```
...
write.csv(subset(qcew, series == "Establishments" & 
                       area == "North Central" & 
                       naicsector %in% c(311,323,332,333,334,336,337,339)),"slide16.csv", 
          row.names = FALSE)
```
...
write.csv(B23001_sum,"slide17.csv",row.names = FALSE)
```
...
write.csv(homeownership.tract,"slide18.csv",row.names = FALSE)
```
...
write.csv(appeals,"slide19.csv",row.names = FALSE)
```
...
write.csv(subset(totalcrime, Town %in% c("Bridgeport","Hartford","New Haven","New Britain","Stamford","Waterbury")),"slide20.csv",row.names = FALSE)
```
...
write.csv(voting,"slide21.csv",row.names = FALSE)
```
